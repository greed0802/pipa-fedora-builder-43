...
sub=$(awk '/^SUBLEVEL =/{print $3}' "$TREE/Makefile")
if [[ $kver != 7.1 && $kver != 7.2 ]] || { [[ $kver == 7.1 ]] && [[ ${sub:-0} -lt 2 ]]; }; then
  echo "Refusing Linux $kver.$sub. Need 7.1.2+ (7.1.0-pipa-cam has cameras and no /dev/snd PCM)." >&2
  exit 1
fi
